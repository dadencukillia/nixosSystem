...
export function OutputAudio() {
  ...
  const currentSpeaker = wp.defaultSpeaker;
  const iconName = createBinding(currentSpeaker, "volumeIcon");
  const deviceName = createBinding(currentSpeaker, "description");
  const mute = createBinding(currentSpeaker, "mute");
  const volume = createBinding(currentSpeaker, "volume");
  ...
  scrollController.connect("scroll", (_s, _dx, dy) => {
    ...
    currentSpeaker.set_volume(volume() - dy / 25);
